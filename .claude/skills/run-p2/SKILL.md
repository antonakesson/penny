---
name: run-p2
description: Build, run, and drive the p2 idle game (static HTML/JS, no build step). Use when asked to start p2, screenshot its UI, or interact with the running game — activate the rummage slot, seed inventory state, check drops.
---
...
| `seed-save <json>` | write a save to `localStorage` merged over `{slots:[],inventory:{}}` — see Gotchas, ordering matters |
...
- **`seed-save` ordering.** `localStorage` isn't readable before a
  ...
  at boot).
- **`seed-save`'s version field is hardcoded** in the driver, tracking
  `storage.js`'s `SAVE_VERSION`. If that constant bumps, update the
  driver too or seeded saves get silently discarded as
  version-mismatched (no migration chain exists — any mismatch is a
  hard wipe, see `storage.js`).
- **Getting Rusty Hook (or any specific drop) without grinding RNG**:
  use `seed-save {"inventory":{"rustyHook":2}}` rather than clicking
  the slot repeatedly — the drop table is weighted/random, not
