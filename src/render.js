...
import { RESOURCE_LABEL, INVENTORY_LABEL, EQUIPPED_LABEL, STATS_TITLE, SLOT_LABEL, STATUS_LABEL } from './flavor.js';
...
function render(state) {
  ...
  for (const wrapper of qsa('.slot')) {
    ...
    for (const option of select.options) {
      ...
      option.textContent = route ? `${targetZone.name} (${route.cost}, ${route.timeMs / 1000}s)` : `${targetZone.name}`;
    }
  }
}
